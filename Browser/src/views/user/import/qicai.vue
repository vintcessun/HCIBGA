<template>
  <div class="import-page">
    <a-card class="general-card" :title="$t('user.import.qicai.title')">
      <a-upload
        :file-list="fileList"
      ...
      >
        <template #upload-button>
          <div class="upload-area">
            ...
            <div class="upload-text">{{ $t('user.import.qicai.upload.text') }}</div>
          </div>
        </template>
      </a-upload>
...
const beforeUpload = (file: File) => {
  ...
  if (!isLt10M) {
    Message.error(t('user.import.qicai.error.fileSize'))
    return false
  }
  return true
}
...
const handleSubmit = async () => {
  if (uploadedFiles.value.length === 0) {
    Message.error(t('user.import.qicai.error.noFile'))
    return
  }
  uploading.value = true
  try {
    Message.success(t('user.import.qicai.success.submit'))
    fileList.value = []
    uploadedFiles.value = []
  } catch (error) {
    Message.error(t('user.import.qicai.error.submit'))
  } finally {
